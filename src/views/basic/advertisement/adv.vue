  ...
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column align="center" label="上架时间">
        <template scope="scope">
          <span>{{scope.row.addTime | dateTimeFormat}}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="下架时间">
        <template scope="scope">
          <span>{{scope.row.offTime | dateTimeFormat}}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="命令编码">
        <template scope="scope">
    ...
    <el-dialog :title="textMap[dialogStatus]" size="tiny" :visible.sync="dialogFormVisible">
      <el-form class="small-space" autoComplete="on" :model="themeBean" :rules="themeRules" ref="themeForm"
               label-position="left" label-width="70px" style='width: 60%; margin-left:50px;'>
